 ...
 * @since 0.0.1
 */
public class BorderParser implements StyleParser {
    private static final Logger log = LoggerFactory.getLogger(BorderParser.class);
    ...
    public Map<String, String> parse(Map<String, String> style) {
        ...
        for (String pos : new String[]{null, TOP, RIGHT, BOTTOM, LEFT}) {
            // border[-attr]
            if (pos == null) {
                setBorderAttr(mapRtn, null, style.get(BORDER));
                setBorderAttr(mapRtn, null, style.get(BORDER + "-" + COLOR));
                setBorderAttr(mapRtn, null, style.get(BORDER + "-" + WIDTH));
                setBorderAttr(mapRtn, null, style.get(BORDER + "-" + STYLE));
            }
            // border-pos[-attr]
            else {
    ...
    public void apply(Cell cell, CellStyle cellStyle, Map<String, String> style) {
        for (String pos : new String[]{TOP, RIGHT, BOTTOM, LEFT}) {
            ...
            HSSFColor poiColor = StyleUtils.parseColor((HSSFWorkbook) cell.getSheet().getWorkbook(), style.get(colorAttr));
            if (poiColor != null) {
                try {
            ...
            // width
            int width = StyleUtils.getInt(style.get(BORDER + "-" + pos + "-" + WIDTH));
    ...
    private void setBorderAttr(Map<String, String> mapBorder, String pos, String value) {
        if (StringUtils.isNotBlank(value)) {
            ...
            for (String borderAttr : value.split("\\s+")) {
                if ((borderColor = StyleUtils.processColor(borderAttr)) != null) {
                    setBorderAttr(mapBorder, pos, COLOR, borderColor);
                } else if (StringKit.isNumeric(borderAttr)) {
