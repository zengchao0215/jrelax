...
function Toolbar(calendar, toolbarOptions) {
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							// classes = [
							// 	'btn btn-primary fc-' + buttonName + '-button',
							// 	tm + '-button',
							// 	tm + '-state-default'
							// ];
				...
				if (groupChildren.length > 1) {
					groupEl = $('<div/>');
					if (isOnlyButtons) {
						groupEl.addClass('fc-button-group btn-group');
					}
	...
	function activateButton(buttonName) {
		if (el) {
            ...
            el.find('.fc-' + buttonName + '-button').addClass('active');
		}
	}
	...
	function deactivateButton(buttonName) {
		if (el) {
			...
			el.find('.fc-' + buttonName + '-button').removeClass('active');
		}
	}
